...
class MySQL
{
	...
	private $db_host    = SITE_DB_HOST; // server name
	private $db_user    = SITE_DB_USER;          // user name
	private $db_pass    = SITE_DB_PASS;          // password
	private $db_dbname  = SITE_DB;          // database name
	...
	*/
	...
	public function InsertRow($tableName, $valuesArray) {
		...
		if (! $this->IsConnected()) {
			...
			// Execute the query
			@mysqli_query($this->mysql_link,"set names 'utf8'");
	 ...
	 */
	public function QueryArray($sql, $resultType = MYSQLI_ASSOC) {
		$this->Query($sql);
		if (! $this->Error()) {
