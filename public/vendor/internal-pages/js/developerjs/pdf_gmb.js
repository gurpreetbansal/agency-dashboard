...
function Reviews(campaignId,page){
	$.ajax({
		type:"GET",
		url:$('.base_url').val()+"/ajax_get_gmb_pdf_reviews",
	...
	$.ajax({
		...
		success:function(response){
			...
			if(response['status'] == 'error'){
				Command: toastr["success"]('Error, please try again later.');
			}
...
function CustomerSearchData(campaignId,key,value){
	$.ajax({
		type:"GET",
		url:$('.base_url').val()+"/ajax_get_Customer_search_pdf_data",
		data:{campaignId: campaignId,key:key,value:value},
		dataType:'json',
		success:function(result){
			if(result['status'] == 1){
				$('.direct_search').html(result['direct']);
				$('.discovery_search').html(result['discovery']);
				$('.branded_search').html(result['branded']);
			}
		}
	});
}