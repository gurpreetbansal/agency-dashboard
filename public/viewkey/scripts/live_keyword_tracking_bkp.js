  ...
  $.ajax({
    ...
    success: function(response) {
      ...
      if(response['lifetime'] > 0){
        $('.keywords_up').addClass('green');
        $('.keywords_up').removeClass('red');
      }else if(response['lifetime'] < 0){
        $('.keywords_up').addClass('red');
        $('.keywords_up').removeClass('green');
      }else{
        $('.keywords_up').removeClass('red');
        $('.keywords_up').removeClass('green');
      }
      $('.top-three').html(response['three'] + '/' + response['total']);

       if(response['three'] > 0){
        $('.top-three').addClass('green');
        $('.top-three').removeClass('red');
      }else if(response['three'] < 0){
      ...
      $('.top-ten').html(response['ten'] + '/' + response['total']);

      if(response['ten'] > 0){
        $('.top-ten').addClass('green');
        $('.top-ten').removeClass('red');
      }else if(response['ten'] < 0){
      ...
      $('.top-twenty').html(response['twenty']+ '/' + response['total']);
      if(response['twenty'] > 0){
        $('.top-twenty').addClass('green');
        $('.top-twenty').removeClass('red');
      }else if(response['twenty'] < 0){
      ...
      $('.top-thirty').html(response['thirty']+ '/' + response['total']);
      if(response['thirty'] > 0){
        $('.top-thirty').addClass('green');
        $('.top-thirty').removeClass('red');
      }else if(response['thirty'] < 0){
      ...
      $('.top-hundred').html(response['hundred']+ '/' + response['total']);
      if(response['hundred'] > 0){
        $('.top-hundred').addClass('green');
        $('.top-hundred').removeClass('red');
      }else if(response['hundred'] < 0){
...
function LiveKeywordTrackingList(campaign_id, column_name, order_type, limit, page, query,key) {
  
  $('.project-search').removeClass('ajax-loader');
  $.ajax({
    url: BASE_URL + '/ajax_live_keyword_list',
    type: 'GET',
    data: {campaign_id, column_name, order_type, limit, page, query,key},
    success: function(response) {
  ...
  $.ajax({
    ...
    type: 'GET',
    data: {campaign_id, column_name, order_type, limit, page, query},
    success: function(response) {
...
/*$(document).ready(function() {
  ...
  LiveKeywordTrackingList($('.campaignID').val(), column_name, order_type, limit, page, query,key);
});*/
...
$(document).on('click','.liveKeyword_sorting',function(e){
 ...
 LiveKeywordTrackingList($('.campaignID').val(), column_name, reverse_order, limit, page, query,key);
