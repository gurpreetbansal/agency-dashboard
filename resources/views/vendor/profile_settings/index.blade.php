								...
								<table>
										...
										<td>
											...
											if(isset($user_package) && !empty($user_package)){
												if($user_package->subscription_type == 'year'){
													echo $user_package->package->name .' ('.$currency.($user_package->price/12).'/month)';
												}else{
													echo $user_package->package->name .' ('.$currency.$user_package->price.'/month)';
												}
											}
											?>
										</td>
									...
									<tr>
										...
										<td @if($user->subscription_status==1) colspan="2" @endif>
											...
											<a href="javascript:;" class="cancel_subscription" data-id="{{Auth::user()->id}}" uk-toggle="target: #cancel-feedback" type="button"><input type="button" class="btn btn-border red-btn-border" value="Cancel"></a>
											@else
				...
				<div>
					<div class="account-form-box">
						<div class="account-form-box-head">
							...
							<a href="{{url('/stripe-download-excel/'.$package_info->id)}}" target="_blank" class="btn btn-sm green-btn">
								<img src="{{URL::asset('public/vendor/internal-pages/images/excel-icon.png')}}"> Download
							</a>
							@endif
						</div>
									...
									<table>
										...
										<tbody>
											...
											<tr>
												...
												<td>$ {{number_format(($invoice->amount_paid/100),2) .'/month'}}</td>
											...
											<tr>
												...
												<td> <i class="fa fa-inr" aria-hidden="true"></i> {{number_format(($invoice->amount_paid),2).'/month'}} </td>
