...
<article class="<?php if($value->review_reply!=''){ echo 'hasReply';}?>">
	...
	if(strlen($value->comment) > 200){
		echo substr($value->comment,0,200).'<span class="ellipsis">...</span><span class="moreText">'.substr($value->comment,200).'</span> <a class="more" href="javascript:;">read more</a>' ;
	}else{
		echo $value->comment;
	}
	?></p>
	...
	<article class="reply">
		<div class="rating">
			{{App\GmbLocation::calculate_weeks($value->reply_update_time)}}
		</div>
		<p>{{$value->review_reply}}</p>
	</article>
	@endif
</article>
@endforeach
