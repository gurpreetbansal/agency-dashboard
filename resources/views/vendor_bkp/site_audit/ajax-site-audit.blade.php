...
$scraped_page = App\BackLinksData::curl_data($urlsSource . $iswww .$summaryTask['domain_info']['name']);
$title = App\BackLinksData::scrape_between($scraped_page, "<title>", "</title>");
    ...
    <div class="elem-flex">
        <div class="elem-start">
            ...
            <div class="score-for">
                ...
                <a href="javascript:;" uk-toggle="target: #offcanvas-pagecode" data-url="{{ $urlsSource . $iswww .$summaryTask['domain_info']['name'] }}/" data-title="{{$title}}" class="btn btn-sm blue-btn viewsource"><i class="fa fa-code"></i> View page code</a>
                <a href="{{url('/audit-pages/'.$campaign_id)}}" class="btn btn-sm btn-border blue-btn-border">View issues</a>
            </div>
        </div>
                ...
                <ul>
                    <li>
                        ...
                        <div class="crawled-pages">
                            {{ $summaryTask['crawl_status']['pages_crawled'] .' /' }}
                            <select class=" audit_crawl_pages" data-width="50px" id="audit_crawl_pages">
                                <option value="50" {{$summaryTask['crawl_status']['max_crawl_pages'] == 50?'selected':''}}>50</option>
                                <option value="100" {{$summaryTask['crawl_status']['max_crawl_pages'] == 100?'selected':''}}>100</option>
                                <option value="500" {{$summaryTask['crawl_status']['max_crawl_pages'] == 500?'selected':''}}>500</option>
                                <!-- <option value="1000" {{$summaryTask['crawl_status']['max_crawl_pages'] == 1000?'selected':''}}>1000</option>
                                    <option value="10000" {{$summaryTask['crawl_status']['max_crawl_pages'] == 10000?'selected':''}}>10000</option> -->
                            </select>
                        </div>
                    </li>
                    <li>
    ...
    <div class="audit-box-body">
        ...
        <div class="audit-issues">
            <ul class="uk-subnav uk-subnav-pill" uk-switcher="connect: .auditIssuesContainer">
                <li><a href="javascript:;">All Issues</a></li>
                <li><a href="javascript:;">Critical Errors</a></li>
                <li><a href="javascript:;">Warnings</a></li>
                <li><a href="javascript:;">Notices</a></li>
                <li><a href="javascript:;">Zero Issues</a></li>
            </ul>
            <div class="content">
                        ...
                        <table>
                            <tbody>
                                @foreach($errorsListing['critical'] as $keyName => $valueName)
                                    ...
                                    <?php   $issuebreakcount++; 
                                    $CriticalErrors .= '<tr><td class="issue-type critical"><a href="'. url('/audit-pages/'.$campaign_id.'?filter='.$keyName) .'">' . trim($auditLevel[$keyName])  .'</a></td><td>'. $valueName .' '.$pages.'</td><td>'. $keylebel . $keyValue .'</td> <td> <a href="javascript:;" class="btn btn-sm btn-border blue-btn-border sidedrawererror" data-type="critical" data-value="'.$keyName.'" uk-toggle="target: #offcanvas-flip" ><i class="fa fa-question-circle" aria-hidden="true"></i> How to fix</a></td> </tr>';
                                    ?> 
                                    @else
                                        <?php
