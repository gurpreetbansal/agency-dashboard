                            ...
                            <form id="project_general_settings" enctype="multipart/form-data">
                                ...
                                <div class="form-dashboards">
                                    ...
                                    <div class="form-summary">
                                        ...
                                        <div class="form-group">
                                            <textarea id="summarydata" name="summarydata" cols="20" class="form-control genralSettings" placeholder="Write your message.." >{{@$summary->edit_section}}</textarea>
                            ...
                            <article class="<?php if(!empty($project_detail->google_account_id) && !empty($project_detail->google_analytics_id)){ echo 'connected'; }?>" id="ProjectSettings-analytics">
                                ...
                                <?php }elseif(!empty($project_detail->google_account_id) && !empty($project_detail->google_analytics_id)){ ?>
                                    ...
                                    <div>
                                        <div class="connected-content">
                                            <ul>
                                                <li uk-tooltip="title:{{$project_detail->get_analytics_connected_email($project_detail->google_account_id)}}; pos:top-center"><big>{{$project_detail->get_analytics_connected_email($project_detail->google_account_id)}}</big> Connected Email</li>
                                                <li uk-tooltip="title:{{$project_detail->getAnalyticsAccount($project_detail->google_analytics_id)}}; pos:top-center"><big>{{$project_detail->getAnalyticsAccount($project_detail->google_analytics_id)}}</big> Account</li>
                                                <li uk-tooltip="title:{{$project_detail->getAnalyticsAccount($project_detail->google_property_id)}}; pos:top-center"><big>{{$project_detail->getAnalyticsAccount($project_detail->google_property_id)}}</big> Property</li>
                                                <li uk-tooltip="title:{{$project_detail->getAnalyticsAccount($project_detail->google_profile_id)}}; pos:top-center"><big>{{$project_detail->getAnalyticsAccount($project_detail->google_profile_id)}}</big> Profile</li>
                                            </ul>
                                        </div>
                ...
                if (in_array(3, $types)){
                    ...
                    <article class="<?php if(!empty($project_detail->gmb_id) && !empty($project_detail->gmb_id)){ echo 'connected'; }?>" id="ProjectSettings-gmb">
                        ...
                        <?php if(!empty($project_detail->gmb_id) && !empty($project_detail->gmb_id)){?>
                           <div>
                            ...
                            <a href="javascript:;" class="edit-btn" data-pd-popup-open="projectSettingGmbPopup" id="SettingsAdwordsBtnId"><i class="fa fa-edit"></i></a>
                    ...
                    <?php }else{?>
                        <div>
                            ...
                            <a href="javascript:;" class="btn btn-border blue-btn-border" data-pd-popup-open="projectSettingGmbPopup" id="SettingsAdwordsBtnId"><?php if(!empty($project_detail->gmb_id)){ echo 'Connected'; }else{echo "Connect";}?></a>
                        </div>
                    <?php } ?>
                </article>
            <?php } ?>
